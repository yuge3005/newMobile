class SlotBackGroundSetting{
	...
	private static drawBackgroundOn( target: egret.Sprite ):void{
		GraphicTool.drawRect( target, new egret.Rectangle( 0, 0, SlotBackGroundSetting.gameMask.width, SlotBackGroundSetting.gameMask.height ), this.bgColor );
	}
	...
	private static buildBGItemsByArray( target: egret.Sprite, mcf: egret.MovieClipDataFactory ): Array<egret.DisplayObject>{
		...
		for( let i: number = 0; i < bgItemNames.length; i++ ){
			...
			if( RES.getRes( SlotMachine.getAssetStr( bgItemNames[i]["name"] ) ) ){
				sp = Com.addBitmapAt( target, SlotMachine.getAssetStr( bgItemNames[i]["name"] ), bgItemNames[i]["x"], bgItemNames[i]["y"] );
				sp.name = SlotMachine.getAssetStr(  bgItemNames[i]["name"] );
			}
			else{
