class BingoBackGroundSetting{
	...
	private static buildBGItemsByArray( target: egret.Sprite, mcf: egret.MovieClipDataFactory ): Array<egret.DisplayObject>{
		...
		for( let i: number = 0; i < bgItemNames.length; i++ ){
			...
			if( RES.getRes( MultiPlayerMachine.getAssetStr( bgItemNames[i]["name"] ) ) ){
				sp = Com.addBitmapAt( target, MultiPlayerMachine.getAssetStr( bgItemNames[i]["name"] ), bgItemNames[i]["x"], bgItemNames[i]["y"] );
				sp.name = MultiPlayerMachine.getAssetStr(  bgItemNames[i]["name"] );
			}
			else{
